<div class="task-board color-<?= $task['color_id'] ?> <?= $task['date_modification'] > time() - $board_highlight_period ? 'task-board-recent' : '' ?>">
    ...
    <?= $this->url->link('#'.$task['id'], 'TaskController', 'readonly', ['task_id' => $task['id'], 'token' => $project['token']]) ?>
    ...
    <div class="task-board-title">
        <?= $this->url->link($this->text->e($task['title']), 'TaskController', 'readonly', ['task_id' => $task['id'], 'token' => $project['token']]) ?>
    </div>
